...
import {GetItemLocalStorage,GetDataApiCalls} from '../Services'
  ...
  awards: [
    {
      ...
      reason: "Exceptional teaching performance",
    },
    {
      id: 2,
      year: 2019,
      name: "Best Research Paper Award",
      reason: "Innovative research contribution",
    },
  ],
};
...
const InstructorProfile = () => {
  const { id } = useParams();
  const[Instructor,setInstructor]=useState(dummyInstructor)
  if(!id)
  {
    let instructor=GetItemLocalStorage('user');
  }
useEffect(() =>{
  //fetch data from backend
  const fetchData=async()=>{
    let data=await GetDataApiCalls('instructorData/'+id)
    setInstructor(data)
  }
  fetchData();
        ...
        <div className="col-md-6">
          <div className="card">
            <div className="card-body">
              ...
              <p>
                <strong>Email:</strong> {dummyInstructor.email}
              </p>
              <p>
                <strong>Department:</strong> {dummyInstructor.department}
              </p>
              <p>
                <strong>Type:</strong> {dummyInstructor.type}
              </p>
              <p>
                <strong>Salary:</strong> {dummyInstructor.salary}
              </p>
              <p>
                <strong>Phone Number:</strong> {dummyInstructor.phone_number}
              </p>
              <p>
                <strong>Performance Score:</strong>{" "}
                {dummyInstructor.performance_score}
              </p>
            </div>
          </div>
        </div>
